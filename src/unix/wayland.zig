...
pub const Window = struct {
    ...
    fn pushKeyEvent(self: *Window, key: u32, comptime event: wio.EventType) void {
        // Layout-aware mapping via XKB keysym (Y/Z swap and the position of
        // "-", "+" and "/" differ per layout). Falls back to the physical
        // keycode when XKB is unavailable or the keysym is not in the table.
        const sym = if (xkb_state) |state| c.xkb_state_key_get_one_sym(state, key + 8) else 0;
        const button = keysym.toButton(sym) orelse keyToButton(key);
        if (button) |b| {
